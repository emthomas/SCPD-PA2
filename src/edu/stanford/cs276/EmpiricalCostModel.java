...
public class EmpiricalCostModel implements EditCostModel{
	...
	public double editProbability(String original, String R, int distance) {
		...
		char edit = editType[0].charAt(0);
			if(edit==ins) {
				answer = (double)(INS.count(x.charAt(0)+x)+1)/(COUNT.count(String.valueOf(x.charAt(0)))+26);
			} 
			else if(edit==del) {
				answer = (double)(DEL.count(w)+1)/(COUNT.count(w)+26);
				//DEL[alphabet.indexOf(x)][alphabet.indexOf(y)]++;
			} 
			else if(edit==sub) {
				answer = (double)(SUB.count(x+w)+1)/(COUNT.count(w)+26);
				//SUB[alphabet.indexOf(x)][alphabet.indexOf(y)]++;
			} 
			else if(edit==trans) {
				answer = (double)(TRANS.count(w)+1)/(COUNT.count(w)+26);
				//TRANS[alphabet.indexOf(x)][alphabet.indexOf(y)]++;
			} 
